...
# State helpers (local FS by default; GCS only if STATE_BUCKET set)
...
def _resolve_band_from_policy(default_band: float = 0.05) -> float:
    """
    ...
    try:
        ...
        if hits:
            try:
                ...
                pass
            # ensure note and halted
            cfg = result2.setdefault("config", {})
            cfg["halted"] = True
            result2["note"] = "ConstraintHit: " + ", ".join(h["type"] for h in hits)
        return result2
    except Exception as e:
        # Fallback: try last saved prices in state, otherwise public spot
        prices = read_json("state/latest_prices.json", default=None)

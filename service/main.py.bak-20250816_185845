...
def plan(refresh: int = 0, pair: Optional[List[str]] = Query(default=None)):
    ...
    Optional what-if overrides: /plan?pair=BTC-USD=125000&pair=SOL-USD=177
    """
...
def apply_paper(commit: int = 0, refresh: int = 0, x_app_key: Optional[str] = Header(None)):
    """
    Stateless paper execution with state in GCS.
    - Loads balances from gs://$STATE_BUCKET/state/balances.json (or seeds from plan's balances)
    - Applies current plan actions in memory
    - If commit=1, writes:
        * state/balances.json
        * trades/YYYYMMDD.jsonl (appended)
        * plans/plan_<ts>_<runid>.json (snapshot)
    """
